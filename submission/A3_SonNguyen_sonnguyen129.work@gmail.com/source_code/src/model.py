...
class TransientExhausted(RuntimeError):
    """Raised when complete_many ran out of retries on transient errors."""


class PermanentRequestError(RuntimeError):
    """Raised on non-retryable API errors (4xx) — surface the bug, don't zero it."""
...
class QwenInference:
    ...
    # ------------------------------------------------------------------
    # Phase-3 additions (add-only; nothing above this line changed)
    # ------------------------------------------------------------------
    ...
    def complete_many(
        ...
        *,
        ...
        """
        ONE /v1/completions request returning n choices (choices share the
        prefill server-side). Unlike _complete(), this reads ALL choices and
        NEVER silently returns "" — transient failures are retried with
        exponential backoff + jitter and, if exhausted, raised as
        TransientExhausted so the caller can re-queue instead of zeroing.
        """
        ...
        if self._client is None:
            raise RuntimeError("Call load() before complete_many().")
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                usage = response.usage
                input_tokens = usage.prompt_tokens if usage else self.count_tokens(prompt)
                completion_total = usage.completion_tokens if usage else 0
                per_choice = completion_total // max(1, len(response.choices))
                results: list[GenerationResult] = []
                for choice in response.choices:
                    ...
                    results.append(
                        GenerationResult(
                            raw_output=raw_text,
                            predicted_answer=extract_answer(raw_text),
                            input_tokens=input_tokens,
                            output_tokens=per_choice or self.count_tokens(raw_text),
                        )
                    )
                return results
            ...
            if attempt < max_attempts:
                sleep_s = min(backoff_cap, backoff_base ** attempt) * _random.uniform(0.5, 1.5)
                logger.warning(
                    "complete_many transient failure (attempt %d/%d): %s — retrying in %.1fs",
                    attempt, max_attempts, last_exc, sleep_s,
                )
                _time.sleep(sleep_s)

        raise TransientExhausted(
            f"complete_many exhausted {max_attempts} attempts: {last_exc}"
        ) from last_exc
    ...
    def assert_served_model(self, expected: str) -> None:
        """Fail fast if the endpoint is serving a different model (swap guard)."""
        if self._client is None:
            raise RuntimeError("Call load() before assert_served_model().")
        served = [m.id for m in self._client.models.list().data]
        if expected not in served:
            raise RuntimeError(
                f"Endpoint is serving {served}, expected {expected!r}. "
                "Swap the Cerebrium app to the right model first."
            )
